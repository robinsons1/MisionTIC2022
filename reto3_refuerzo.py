...
def main():
    ...
    for i in range(m):
        ...
        if 0 < depart <= n:
            if type_new == 'a':
                acum_ant[depart - 1]   += max(0, math.ceil((area - area_old * ant_old) / 11400))  # Aqui debes realizar el calculo de las antenas de tipo a con su rango respectivo
                ...
                acum_ant[depart - 1] += max(0, math.ceil((area - area_old * ant_old) / 14700))  # Aqui debes realizar el calculo de las antenas de tipo d con su rango respectivo
            elif type_new == 'e':
                acum_ant[depart - 1] += max(0, math.ceil((area - area_old * ant_old) / 38000))  # Aqui debes realizar el calculo de las antenas de tipo e con su rango respectivo
